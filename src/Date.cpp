...
namespace minirisk {
    ...
    const std::array<unsigned, 12> Date::days_leap_ytd{ {0,31,60,91,121,152,182,213,244,274,305,335} };
    ...
    */
    ...
    void Date::check_valid(unsigned s) {
        MYASSERT(s >= 0 && s <= 109572, "The serial must be a integer between 0 and 109572, got " << s << ".");
    }

    unsigned Date::day_of_year() const
    {
        unsigned day = get_serial();
        unsigned year = 1900;
        while (day >= 366) {
